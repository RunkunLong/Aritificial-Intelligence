...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    while iteration<iterations:
        ...
        for state in mdp.getStates():
           ...
           if not actions:
             ...
             values[state]=max(self.getQValue(state,action) for action in actions)
        self.values=values
        iteration+=1
  ...
  def getQValue(self, state, action):
    ...
    for possibleState, probability in self.mdp.getTransitionStatesAndProbs(state, action):
      Qvalue = Qvalue + probability * (self.mdp.getReward(state, action, possibleState) + (self.discount * self.getValue(possibleState)))
    return Qvalue
